...
#define VS_TOUCH_COUNT_EXPIRED 20 //should be about 2 sedconds to answer
...
void verifySequence_disable()
{
    ...
    VS_State = VS_init_st;
    //
    inputErrorFlag = false; //reset flags
    touchCountExpiredFlag = false; //reset flags
}
...
void verifySequence_tick()
{
    if (verifySequenceEnable) //if its turned on
    {
        ...
        case VS_verify_input_st:
            ...
            else if (VS_index >= globals_getSequenceIterationLength() - 1) //needs to be -1 because index start at zero
                VS_State = VS_complete_st;
